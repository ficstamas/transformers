...
from tensorflow_text import LSHTokenizer as LSHTokenizerLayer
from tensorflow_text import FastLSHTokenizer, ShrinkLongestTrimmer, case_fold_utf8, combine_segments, pad_model_inputs

from .tokenization_lsh import LSHTokenizer


class TFLSHTokenizer(tf.keras.layers.Layer):
    """
    This is an in-graph tokenizer for LSH. It should be initialized similarly to other tokenizers, using the
    ...
    Args:
        ...
        use_fast_lsh_tokenizer (`bool`, *optional*, defaults to `True`):
            If True, will use the FastLSHTokenizer class from Tensorflow Text. If False, will use the LSHTokenizer
            class instead. LSHTokenizer supports some additional options, but is slower and cannot be exported to
            TFLite.
    """
    ...
    def __init__(
        ...
        use_fast_lsh_tokenizer: bool = True,
        **tokenizer_kwargs,
    ):
        super().__init__()
        if use_fast_lsh_tokenizer:
            self.tf_tokenizer = FastLSHTokenizer(
            ...
            self.tf_tokenizer = LSHTokenizerLayer(
    ...
    def from_tokenizer(cls, tokenizer: "PreTrainedTokenizerBase", **kwargs):  # noqa: F821
        """
        Initialize a `TFLSHTokenizer` from an existing `Tokenizer`.

        Args:
            tokenizer (`PreTrainedTokenizerBase`):
                The tokenizer to use to initialize the `TFLSHTokenizer`.

        Examples:

        ```python
        from transformers import AutoTokenizer, TFLSHTokenizer

        tokenizer = AutoTokenizer.from_pretrained("ficsort/lsh-base")
        tf_tokenizer = TFLSHTokenizer.from_tokenizer(tokenizer)
        ```
        """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, os.PathLike], *init_inputs, **kwargs):
        """
        Instantiate a `TFLSHTokenizer` from a pre-trained tokenizer.
        ...
        ```python
        from transformers import TFLSHTokenizer

        tf_tokenizer = TFLSHTokenizer.from_pretrained("ficsort/lsh-base")
        ```
        """
        try:
            tokenizer = LSHTokenizer.from_pretrained(pretrained_model_name_or_path, *init_inputs, **kwargs)
        except:  # noqa: E722
            from .tokenization_lsh_fast import LSHTokenizerFast

            tokenizer = LSHTokenizerFast.from_pretrained(pretrained_model_name_or_path, *init_inputs, **kwargs)
