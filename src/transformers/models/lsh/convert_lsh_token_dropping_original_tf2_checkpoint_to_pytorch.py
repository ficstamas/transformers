...
"""
This script converts a lm-head checkpoint from the "Token Dropping" implementation into a PyTorch-compatible LSH
...
from transformers import LSHConfig, LSHForMaskedLM
from transformers.models.lsh.modeling_lsh import (
    LSHIntermediate,
    LSHLayer,
    LSHOutput,
    LSHPooler,
...
def convert_checkpoint_to_pytorch(tf_checkpoint_path: str, config_path: str, pytorch_dump_path: str):
    ...
    config = LSHConfig.from_json_file(config_path)
    model = LSHForMaskedLM(config)

    # Layers
    for layer_index in range(0, config.num_hidden_layers):
        layer: LSHLayer = model.lsh.encoder.layer[layer_index]

        # Self-attention
        self_attn: LSHSelfAttention = layer.attention.self
        ...
        self_output: LSHSelfOutput = layer.attention.output
        ...
        # Intermediate
        intermediate: LSHIntermediate = layer.intermediate
        ...
        # Output
        lsh_output: LSHOutput = layer.output

        lsh_output.dense.weight.data = get_encoder_layer_array(layer_index, "_output_dense/kernel")
        lsh_output.dense.bias.data = get_encoder_layer_array(layer_index, "_output_dense/bias")

        lsh_output.LayerNorm.weight.data = get_encoder_layer_array(layer_index, "_output_layer_norm/gamma")
        lsh_output.LayerNorm.bias.data = get_encoder_layer_array(layer_index, "_output_layer_norm/beta")

    # Embeddings
    model.lsh.embeddings.position_embeddings.weight.data = get_encoder_array("_position_embedding_layer/embeddings")
    model.lsh.embeddings.token_type_embeddings.weight.data = get_encoder_array("_type_embedding_layer/embeddings")
    model.lsh.embeddings.LayerNorm.weight.data = get_encoder_array("_embedding_norm_layer/gamma")
    model.lsh.embeddings.LayerNorm.bias.data = get_encoder_array("_embedding_norm_layer/beta")
    ...
    model.lsh.embeddings.word_embeddings.weight.data = get_masked_lm_array("embedding_table")

    # Pooling
    model.lsh.pooler = LSHPooler(config=config)
    model.lsh.pooler.dense.weight.data: LSHPooler = get_encoder_array("_pooler_layer/kernel")
    model.lsh.pooler.dense.bias.data: LSHPooler = get_encoder_array("_pooler_layer/bias")

    # Export final model
    model.save_pretrained(pytorch_dump_path)

    # Integration test - should load without any errors ;)
    new_model = LSHForMaskedLM.from_pretrained(pytorch_dump_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The config json file corresponding to the LSH model. This specifies the model architecture.",
    )
    parser.add_argument(
        "--pytorch_dump_path",
        type=str,
    ...
    convert_checkpoint_to_pytorch(args.tf_checkpoint_path, args.lsh_config_file, args.pytorch_dump_path)
